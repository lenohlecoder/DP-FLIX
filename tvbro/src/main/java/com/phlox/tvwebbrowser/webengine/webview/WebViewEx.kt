...
open class WebViewEx(context: Context, val callback: Callback, val jsInterface: AndroidJSInterface) : WebView(context) {
    ...
    interface Callback {
        ...
        /** DP-FLIX locked mode: blocks a network resource (including iframes) without touching protected hosts. */
        fun isDpFlixBlockedResource(request: WebResourceRequest): Boolean
        fun onWebViewRenderProcessGone(didCrash: Boolean): Unit
    ...
    init {
        ...
        webViewClient = object : WebViewClient() {
            ...
            override fun shouldInterceptRequest(view: WebView, request: WebResourceRequest): WebResourceResponse? {
                ...
                // DP-FLIX locked mode policy is deliberately evaluated before TV Bro's
                // normal adblock engine. This catches ad/redirect iframes even when the
                // main frame stays on the legitimate stream host. It only blocks URLs
                // explicitly classified by the host policy; everything else continues
                // through unchanged, preserving CDN/video/JS resources.
                if (callback.isDpFlixBlockedResource(request)) {
                    Log.d(TAG, "DP-FLIX blocked resource: ${request.url}")
                    uiHandler.post { callback.onBlockedAd(request.url) }
                    val response = WebResourceResponse("text/plain", "utf-8", "".byteInputStream())
                    response.setStatusCodeAndReasonPhrase(403, "Blocked by DP-FLIX policy")
                    return response
                }
            ...
            override fun onRenderProcessGone(view: WebView, detail: RenderProcessGoneDetail): Boolean {
                callback.onWebViewRenderProcessGone(detail.didCrash())

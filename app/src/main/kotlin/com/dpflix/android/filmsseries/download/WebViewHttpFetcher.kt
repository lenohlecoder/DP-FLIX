...
object WebViewHttpFetcher {
    ...
    /**
     * Récupère une URL binaire (segment vidéo/audio .ts, .m4s...) dans le contexte de
     * la WebView. Le JS lit la réponse en `arrayBuffer()` puis l'encode en base64 (le
     * pont `evaluateJavascript` ne transporte que du texte) ; on décode ce base64 ici
     * côté Kotlin pour reconstituer les octets bruts.
     *
     * Mêmes garanties que [fetchText] : erreurs HTTP remontées en "ERROR:...", même
     * timeout, même dispatch sur le thread UI (requis par WebView).
     */
    suspend fun fetchBytes(
        ...
        suspendCancellableCoroutine { cont ->
            ...
            val js = buildBytesFetchJs(url)
            val runner = Runnable {
                try {
                    webView.evaluateJavascript(js) { raw ->
                        complete {
                            try {
                                val payload = decodeJsStringResult(raw)
                                    ?: throw IllegalStateException("Réponse WebView vide")
    ...
    private fun buildTextFetchJs(url: String): String {
    ...
    /**
     * Fetch binaire : `arrayBuffer()` → parcours par blocs de 8ko (évite les dépassements
     * de pile sur `String.fromCharCode(...bytes)` avec un spread trop large sur gros
     * segments) → `btoa` pour obtenir du base64 transportable en texte.
     */
    private fun buildBytesFetchJs(url: String): String {
        val escaped = escapeForJs(url)
        return """
            (function(){
              return fetch('$escaped', {
    ...
    private fun escapeForJs(url: String): String =
